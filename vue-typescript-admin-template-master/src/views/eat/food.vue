...
import { IFood } from "@/api/food/types";
...
export default class extends Vue {
   ...
   * 弹框回调
   */
  private async dialogSubmit(title: string, params: any) {
    ...
    if (title === "添加") {
        ...
        if (data.msg === "添加成功") { showNotify(4, "创建" + params.title + "成功"); }
      } else if (title.indexOf("修改") !== -1) {
        const { data } = await update(formData);
        console.log(data);
        if (data.msg === "修改成功") {
          showNotify(4, "修改" + params.title + "成功");
        }
      }
  ...
  private btnView(row: any) {
    this.agentEvent.showDialog(`查看【${row.title}】美食`, false, true);
  ...
  private btnEdit(row: any) {
      console.log(row);
    this.agentEvent.showDialog(`修改【${row.title}】美食`, true, false);
    initForm(
      row.id,
      row.title,
      row.content,

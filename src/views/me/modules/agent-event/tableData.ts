...
const columns = {
    ...
    type: {
        ...
        filterTag: (value: string, row: any, column: Record<string, any>) => filterTag(value, row, column),
...
const setScheduleStauts = (row: any) => {
    if (row.schedule <= 20) {
       return colorStatus(1, 2);
    } else if (row.schedule > 20 && row.schedule <= 40) {
        return colorStatus(2, 2);
    } else if (row.schedule > 40 && row.schedule <= 60) {
        return colorStatus(3, 2);
    } else if (row.schedule > 60 && row.schedule <= 80) {
        return colorStatus(4, 2);
    } else if (row.schedule > 80 && row.schedule <= 100) {
 ...
 * @param columns
 */
const filterTag = (value: string, row: any, columns: Record<string, any>) => {
